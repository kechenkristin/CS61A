...
def no_repeats(lnk):
    """ Takes a linked list of numbers lnk and returns a linked list that
    has all of the unique elements of lnk in order that they first appear, 
    but no repeats.

    >>> s1 = Link(5, Link(4, Link(5, Link(4, Link(2, Link(2))))))
    >>> s2 = no_repeats(s1)
    >>> s2
    Link(5, Link(4, Link(2)))
    """
    if lnk is Link.empty:
        return Link.empty
    else:
        return Link(lnk.first, no_repeats(filter_link(lambda x: x != lnk.first), lnk.rest))
