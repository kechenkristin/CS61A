...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(rest, Pair):
            def new_eval(expr): return scheme_eval(expr, env)
            operands = rest.map(new_eval)
        else:
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        def helper(formal, val):
            if formal is nil:
                return
            else:
                Frame.define(new_frame, formal.first, val.first)
                return helper(formal.rest, val.rest)
    ...
    def apply(self, args, env):
        ...
        def convert(lnk):
            if lnk is nil:
                ...
                return [lnk.first] + convert(lnk.rest)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        Frame.define(env, target, to_bind)
        return target
        # END PROBLEM 5
        ...
        "*** YOUR CODE HERE ***"
        f_name = target.first
        f_formals = target.rest
        f_body = expressions.rest
        to_bind = do_lambda_form(Pair(f_formals, f_body), env)
        Frame.define(env, f_name, to_bind)
        return f_name
        # END PROBLEM 9
    else:
